...
vector<Num> Num::gcdExt(Num a, Num b){
    if(b.Compare(Num(0)) == 0){
        vector<Num> m;
        m.push_back(a); m.push_back(Num(1)); m.push_back(Num(0));
        return m;
    }
    vector<Num>gcd = gcdExt(b, a.module(b));
    vector<Num>extGcd;
    extGcd.push_back(gcd[0]); extGcd.push_back(gcd[2]); extGcd.push_back(gcd[1].Min(a.DivOp(b).Mult(gcd[2])));
    return gcd;
...
*/
bool Num::MillerRabinTest(int k){
    int s = 0;
    bool d = true;
    Num n = *this;
    Num c = *this;
    ...
    for(int i = 0; i< k;++i){
        Num a(rand()+2);
        Num x = a.modular_power(c, n);
        if((x.Compare(Num(1)) == 0) || x.Compare(n.Min(Num(1))) == 0){
            continue;
        }
        for(int j = 0; j < s-1; ++j){
            x = x.modular_power(Num(2), n);
            if(x.Compare(Num(1)) == 0)
            {
                return 0;
            }
            if(x.Compare(n.Min(Num(1))) == 0){
                break;
            }
        }
        return 0;
...
void Num::RSA(Num p, Num q, Num m){
    chrono::steady_clock::time_point begin = std::chrono::steady_clock::now();
    Num N = p.Mult(q);
    Num pN = (p.Min(Num(1))).Mult(q.Min(Num(1)));
    Num e(3);
    Num gcdd(0);
    Num* dp = &gcdd;
    Num x(0);
    ...
    Num y(0);
    Num *yp = &y;
    gcdExt(e, pN, dp, px, yp);
    Num modInv =  ((x.module(pN)).Add(pN)).module(pN);
    
    Num c = m.modular_power(e, N);
    
    Num mDeciph = c.modular_power(modInv, N);
    cout << "Deciphered text "; mDeciph.printNum(); cout <<"\n";
    chrono::steady_clock::time_point end = std::chrono::steady_clock::now();
    cout << "Time for "; p.printNum(); cout << " and "; q.printNum(); cout << " is " << chrono::duration_cast<std::chrono::microseconds>(end - begin).count() << "ms" << "\n";
}
void Num::RSAOAEP(){
    chrono::steady_clock::time_point begin = std::chrono::steady_clock::now();
//    int n = log(3557*2579 +1) + 1;
//    int k0 = 32*8;
    int k0 = 64;
//    int k1 = n - k0;
    int k1 = 64;
    ...
    SHA256 G;
    G.hash(r, l);
    unsigned char Gres[l];
    for(int i = 0; i < l/2; ++i){
        Gres[i] = G.hashv[i]%127;
    }
    for(int i = l/2; i < l; ++i){
        Gres[i] = (G.hashv[i/2]/128)%127;
    }
    unsigned char m[] = {'a', 'b', 'c'};
    int mlen = sizeof(m)/sizeof(m[0]);
    unsigned char Mpadded[l];
    for(int i = 0; i < mlen; ++i){
        Mpadded[i] = m[i];
    }
    unsigned char X[l];
    for(int i = 0; i < l; ++i){
        X[i] = Gres[i] ^ Mpadded[i];
    }
    SHA256 H;
    H.hash(X, l);
    unsigned char Hres[l];
    for(int i = 0; i < l/2; ++i){
        Hres[i] = H.hashv[i]%127;
    }
    for(int i = l/2; i < l; ++i){
        Hres[i] = (H.hashv[i/2]/128)%127;
    }
    unsigned char Y[k0];
    for(int i = 0; i < k0; ++i){
        Y[i] = Hres[i] ^ r[i];
    ...
    chrono::steady_clock::time_point end = std::chrono::steady_clock::now();
    cout << "Time for OAEP" << " is " << chrono::duration_cast<std::chrono::microseconds>(end - begin).count() << "ms" << "\n";
}
